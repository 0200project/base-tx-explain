...
Three of those change the answer rather than the setup. Without `ETHERSCAN_API_KEY` the counterparty-history check reports `unavailable` rather than running. Without your own `BASE_RPC_URLS` you are on public endpoints, which rate-limit under concurrency — and when they do, the ETH price behind `gas_paid_usd` falls back to the latest reading instead of the price at the transaction's block; `gas_price_basis` says which was used. Everything else populates with no keys at all.
