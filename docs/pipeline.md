...
| AgentPay MCP (up2itnow0822, solo/small team) | Open-source non-custodial x402 payment layer for AI agents, actively committed this week (SSRF hardening, honest session-revocation docs). Their own get_transaction_history tool, per their README table, only returns recipient/amount/block number for on-chain settlements, no decode of what a swap/bridge action under their spend-policy actually did. Found via a 6-modality discovery sweep; a claimed "NVIDIA NeMo Agent Toolkit integration" in the initial research turned out to be a single closed feature-request issue, not real, caught and dropped before drafting. | Sent: github.com/up2itnow0822/agentpay-mcp/issues/48 | Pending | — | Wait; check thread periodically. Platform prepped a concrete before/after decode example (Circadian's real settlement tx) as a follow-up if they engage, not used in the opener. | No |
